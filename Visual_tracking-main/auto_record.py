...
def main():
    ...
    ap.add_argument("--base", default="http://127.0.0.1:5055", help="pi_collect 服务地址，例如 http://<PiIP>:5055")
    ...
    # 1) 设目标：phase=selected（抓取前“保持在桌面”）
    ok, resp = post(f"{base}/bci/target", json={"id": args.target, "phase": "selected"})
    if not ok or not resp.get("ok", False):
        print(f"[ERR] 设目标失败: {resp}")
        sys.exit(2)
    print(f"[OK] 目标已设为 {args.target}, phase=selected")
    ...
    # 3) 抓取（pi_collect.py 的 /demo_grasp 内部会在抓到后把 phase 设为 confirmed）
    ...
    if not ok or not resp.get("ok", False):
        print(f"[ERR] stop 失败: {resp}")
        sys.exit(4)
    print(f"[OK] stop: {resp.get('file','(no path)')}")

    print("\n[DONE] 本次录制完成。请将物块复位到预定位置，然后再次运行本脚本开始下一段。")
